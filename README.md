...
| GPT 写代码、DeepSeek 做架构，模型用反了 | **GPT 想 → DeepSeek 做 → GPT 审**，三步两模型，刻进规则 |
...
<p align="center">
  <br>
  <b>AI 写代码最大的浪费：用错了模型。</b><br>
  GPT 擅长推理和架构，让它写代码，又慢又贵。<br>
  DeepSeek 执行效率高，让它做架构，出来的东西不敢用。
  <br><br>
</p>

<p align="center">
  <b><code>GPT 想 → DeepSeek 做 → GPT 审</code></b>
  <br><br>
...
| 阶段 | 模型 | 工具 | 定位 |
|------|------|------|------|
| 🧭 需求分析 · 拆计划 · 审查 · 验收 · 复盘 | **GPT 5.5** | Codex | 擅长推理和架构，负责"想"和"审" |
| ⚡ 代码实现 · TDD · 紧急修复 | **DeepSeek** | Claude Code | 执行效率高、输出简洁，负责"做" |

每个 prompt 文件的 frontmatter 都标注了 `model` 字段——在 Obsidian 中一目了然。
...
| # | 阶段 | 一句话 | 模型 |
|---|------|--------|------|
| 00 | 任务分流 | 不知道该干啥？先来这里 | GPT 5.5 |
| 01 | 需求夯实 | AI 反问你，把模糊需求问清楚 | GPT 5.5 |
| 02 | 计划拆解 | 拆成独立 commit，逐个击破 | GPT 5.5 |
| 03 | TDD 实现 | Red → Green → Refactor | DeepSeek |
| 04 | 代码审查 | 正确性 · 安全 · 性能 · 覆盖 | GPT 5.5 |
| 05 | 最终验收 | 全链路验证，确认能上线 | GPT 5.5 |
| 06 | 紧急修复 | 线上炸了？先止血再复盘 | DeepSeek |
| 07 | 代码重构 | 行为不变，只清理结构 | GPT 5.5 |
| 08 | 项目脚手架 | 新项目从 0 到 1 | GPT 5.5 |
| 09 | 任务交接 | 做到一半给别人接 | GPT 5.5 |
| 10 | PR 合并收尾 | PR 模板、测试结果、回滚方式 | GPT 5.5 |
| 11 | 复盘改进 | 事故/Issue 结束后总结 | GPT 5.5 |
| 12 | 环境排障 | 依赖、服务、外部异常 | GPT 5.5 |
...
| 工具 | 模型 | 角色 |
|------|------|------|
| **Claude Code** | DeepSeek | 编码执行、TDD 实现、紧急修复 |
| **Codex** | GPT 5.5 | 架构思考、需求分析、代码审查、验收 |

skill 入口索引（`能力入口索引-skill-entry-index.md`）统一管理三端（Claude Code / Codex / Cursor）的 skill 安装与同步。
